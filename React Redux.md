...
The reducer and action creator were modified to remove an item from an array based on the index of the item. Finish writing the reducer so a new statey is returned with the item at the specific index removed.
...
const immutableReducer = (state = defaultState, action) => {
  switch(action.type) {
    case 'ONLINE':
      // don't mutate state here or the tests will fail
      const stat = Object.assign({}, state)
      stat.status = 'online'
